...
npm run dev
```
> PostgreSQL·Redis를 백그라운드로 띄운 뒤 Spring Boot를 포그라운드로 실행합니다.

### 백엔드만
```bash
npm run docker:up      # PostgreSQL, Redis 시작
...
npm run android:prod   # 프로덕션 빌드 → api.navblind.com 서버
```
...
│   ├── smartwalker-firebase-adminsdk-key.json  # Firebase 키 (git 제외)
...
| 구성 요소 | 기술 |
|---|---|
| Android | Kotlin, Jetpack Compose, TFLite (YOLOv8), ARCore, Hilt |
| Backend | Java 21, Spring Boot 3.5.10, PostgreSQL, Redis |
| 라우팅 | OSRM (self-hosted), Nominatim (self-hosted) |
